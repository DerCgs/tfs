...
class WorkThread : public tbutil::Thread
{
  public:
    WorkThread();
    WorkThread(BlockConsole* blk_console, const std::string& src_ns_addr, const std::string& dest_ns_addr, const int64_t max_traffic) :
      blk_console_(blk_console), src_ns_addr_(src_ns_addr), dest_ns_addr_(dest_ns_addr), traffic_(max_traffic * 1024), destroy_(false)
    {
    }
...
void helper()
{
  std::string options=
    ...
    "-w                 traffic threshold per thread(KB/s), should be positive integer, default 1024, max:8MB/s\n"
    "-t                 thread count\n"
    "-l                 log file, defalut path is tranblk.log\n"
    "-p                 pid file, defalut path is tranblk.pid\n"
    "-k                 log level, optional, default info\n"
    "-d                 daemon\n"
    "-h                 help\n"
    "-v                 version\n";
...
int main(int argc, char* argv[])
{
  ...
  help = src_ns_ip_port.empty() || dest_ns_ip_port.empty() || dest_ds_ip_file.empty()
    || help || ts_input_blocks_file.empty() || traffic_threshold <= 0;
  if (help)
  {
    helper();
    return TFS_ERROR;
  ...
  if (0 == pid)
  {
    ...
    else
    {
      ...
      else if(TFS_SUCCESS != (ret = TfsClientImplV2::Instance()->initialize(NULL)))
      {
        TBSYS_LOG(ERROR, "TfsClientImplV2::Instance()->initialize error");
      ...
      else
      {
        ...
        {
          gworks[i] = new WorkThread(blk_console, src_ns_ip_port, dest_ns_ip_port, traffic_threshold);
        }
